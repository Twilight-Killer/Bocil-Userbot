...
async def dspam_cmd(client, message):
    ...
    if reply:
        try:
            ...
            return await msg.edit(str(error))
        dspam_processes[message.chat.id] = {"count_message": count_message, "count_delay": count_delay, "text_spam": reply.text, "is_cancelled": False}
        for i in range(count_message):
            if message.chat.id not in dspam_processes or dspam_processes[message.chat.id]["is_cancelled"]:
                break
            try:
                await reply.copy(message.chat.id)
        ...
        if len(message.command) < 4:
            ...
            try:
                ...
                return await msg.edit(str(error))
            dspam_processes[message.chat.id] = {"count_message": count_message, "count_delay": count_delay, "text_spam": message.text.split(None, 3)[3], "is_cancelled": False}
            for i in range(count_message):
                if message.chat.id not in dspam_processes or dspam_processes[message.chat.id]["is_cancelled"]:
                    break
                try:
...
async def list_dspam(client, message):
    ...
    for chat_id, process_info in dspam_processes.items():
        try:
            ...
            text += f"- {chat_name} (Penundaan: {process_info['count_delay']} detik, Kata-kata: {process_info['text_spam']})\n"
...
async def cancel_dspam(client, message):
    ...
    # Menghentikan proses dspam dengan mengatur flag untuk dihentikan
    dspam_info = dspam_processes[message.chat.id]
    dspam_info["is_cancelled"] = True
